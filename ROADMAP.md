...
Current state: store operations, event handlers, scalar ABI types, and native testing all work. ERC20 and ERC721 subgraphs are live on The Graph Studio (Arbitrum One).
...
Add `callHandlers` support to:
- `graphite-macros`: `#[handler(call)]` attribute variant
- Manifest: `callHandlers` section with `function` signature matching
- Codegen: generate a typed `RustCallData` struct from the ABI function signature
...
**Status:** no concept of non-fatal errors.
The Graph supports `features: [nonFatalErrors]` in manifests — handlers can log errors without halting indexing. Add a `graphite::error!(msg)` macro that calls `log.log(CRITICAL, msg)` and sets a non-fatal flag.

### 4.6 `ipfs.cat` native mock
**Status:** FFI imported, no local mock.
`mock::set_ipfs_result(cid, bytes)` so IPFS-fetching handlers can be unit tested.
